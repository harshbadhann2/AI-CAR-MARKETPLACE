...
export async function getAdmin() {
  try {
    ...
    if (!userId) return { authorized: false, reason: "not-logged-in" };

    if (!isDatabaseConfigured) {
      // Return demo admin access when DB is not configured
      return { authorized: true, user: { id: "demo", role: "ADMIN", name: "Demo Admin" } };
    }
